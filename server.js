...
function emitOperators(event, data) {
...
const pairing = new PairingManager(path.join(__dirname, 'data', 'operator-pairing.json'));
const OPERATOR_COOKIE = 'jnk_operator';
const PAIRING_COOKIE_MAX_AGE_SEC = 180 * 24 * 60 * 60;
...
function requestIsOperator(req) {
    return isLoopback(req.ip || (req.socket && req.socket.remoteAddress)) ||
        pairing.verify(cookieValue(req.headers && req.headers.cookie, OPERATOR_COOKIE));
}

function requireOperator(req, res, next) {
    if (requestIsOperator(req)) return next();
    res.status(401).json({ ok: false, error: 'Pair this device before using operator controls.' });
}

function pairingClaimAllowed(address) {
    const key = String(address || 'unknown');
    const now = Date.now();
    const recent = (pairingAttempts.get(key) || []).filter(t => now - t < 10 * 60 * 1000);
    recent.push(now);
...
// ── QR operator pairing ──────────────────────────────────────────
// The QR is only generated by localhost or an already-paired device.
// A scanned code is one-use, expires after five minutes, and becomes
// an HttpOnly operator cookie so frontend JavaScript never sees the token.
app.use('/api/pairing', (req, res, next) => {
    res.setHeader('Cache-Control', 'no-store');
    next();
});
app.get('/api/pairing/bootstrap', requireOperator, async (req, res) => {
    try {
        const networks = lanUrls();
        const requestedAddress = String(req.query.address || '');
        const selected = networks.find(network => network.address === requestedAddress) ||
            networks.find(network => /^(WLAN|Wi-?Fi|Ethernet)$/i.test(network.name)) ||
            networks.find(network => /^(10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.)/.test(network.address)) ||
            networks[0];
        const { code, expiresAt } = pairing.createCode();
        const base = selected ? selected.url : 'http://localhost:3000';
        const pairingUrl = base + '/pair.html?code=' + encodeURIComponent(code);
        const qrDataUrl = await QRCode.toDataURL(pairingUrl, {
            width: 320,
            margin: 4,
            color: { dark: '#000000', light: '#ffffff' },
        });
        res.json({ ok: true, code, expiresAt, pairingUrl, qrDataUrl, devices: pairing.listDevices() });
    } catch (e) {
...
app.post('/api/pairing/claim', (req, res) => {
    const address = req.ip || (req.socket && req.socket.remoteAddress);
    if (!pairingClaimAllowed(address)) {
        return res.status(429).json({ ok: false, error: 'Too many attempts. Wait a few minutes and try again.' });
    }
    try {
        const result = pairing.claim(req.body && req.body.code, req.body && req.body.deviceName);
        if (!result) return res.status(400).json({ ok: false, error: 'This pairing code is invalid or has expired.' });
        res.setHeader('Set-Cookie', `${OPERATOR_COOKIE}=${encodeURIComponent(result.token)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${PAIRING_COOKIE_MAX_AGE_SEC}`);
        emitOperators('pairingChanged', { devices: pairing.listDevices() });
        res.json({ ok: true, device: result.device });
    } catch (e) {
        res.status(500).json({ ok: false, error: 'Could not pair this device.' });
    }
});

app.get('/api/pairing/devices', requireOperator, (req, res) => {
...
app.delete('/api/pairing/devices', requireOperator, (req, res) => {
    pairing.revokeAll();
    for (const socket of io.sockets.sockets.values()) {
        if (socket.data.operator && !socket.data.localOperator) {
            socket.emit('authState', { operator: false });
            socket.disconnect(true);
        }
    }
    emitOperators('pairingChanged', { devices: [] });
    res.setHeader('Set-Cookie', `${OPERATOR_COOKIE}=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0`);
    res.json({ ok: true });
});

app.get('/api/config', requireOperator, (req, res) => {
    res.json({ fields: config.getClientView() });
});

app.post('/api/config', requireOperator, (req, res) => {
    try {
...
app.get('/api/network', requireOperator, (req, res) => {
...
app.post('/api/break-ad/upload', requireOperator, (req, res) => {
    try {
...
io.on('connection', async socket => {
    ...
    // The audience-facing break deck remains intentionally read-only and public.
    socket.emit('breakState', breakState.get());
    if (!operator) {
        console.log('Read-only display connected');
        return;
    }

    console.log('Operator connected');

    // Send current OBS status and scene to newly connected operator
    try {
        const currentScene = await obs.call('GetCurrentProgramScene');
    ...
    socket.on('breakMode',   mode => breakState.setMode(mode));
